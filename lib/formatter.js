...
Formatter.prototype.summary = function (summary) {
  ...
  if (this.needDuration) {
    output.push(this.format("duration: " + prettyMs(summary.duration), "cyan"));
  }
  output.push(
    this.format(
...
Formatter.prototype.comment = function (comments) {
  ...
  output.push(
    Object.keys(comments)
      .map(function (name) {
        return (
          this.format("## " + name, "cyan.underline") +
          LF +
          comments[name].join(LF)
        );
      }, this)
      .join(LF + LF)
  );
    ...
    Object.keys(fail)
      .map(function (name) {
        ...
        fail[name].forEach(function (assertion) {
          res.push(
            this.format("  " + symbols.cross + " " + assertion.name, "red")
          );
...
Formatter.prototype.splitter = function (s) {
  return this.format("# " + s + LF, "yellow");
};
